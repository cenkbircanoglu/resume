\documentclass[letter,10pt]{article}
\usepackage[utf8]{inputenc}

%====================
% File:         resume.tex
% Editor:       https://github.com/TimmyChan 
%               https://www.linkedin.com/in/timmy-l-chan/
% Last Updated: Nov 19, 2021
% Notes:        All formatting is done in TLCresume.sty
%               No need to touch that file. 
%               All actual content is in the sections/ folder.
%====================

\usepackage{TLCresume}

%====================
% CONTACT INFORMATION
%====================
\author{Cenk Bircanoglu} % Name Here
\def\phone{(+33)755 27 43 86}
\def\city{Paris, France}
\def\email{cenk.bircanoglu@gmail.com}
\def\LinkedIn{cenkbircanoglu} % linkedin.com/in/______
\def\github{cenkbircanoglu} % github username
\def\scholar{https://scholar.google.com/citations?user=kAGirc8AAAAJ&hl=en}
\def\role{Senior Data Scientist} % JOB TITLE

\begin{document}
\import{sections/}{_header}
\import{sections/}{objective}

\section{Skills}
\import{sections/}{skills}

\section{Technical Experience}
\import{sections/}{experience}

\section{Education}
\import{sections/}{education}

% make a newpage wherever it is a clean break
% \newpage

\section{Activities}
\import{sections/}{activities}


\end{document}
